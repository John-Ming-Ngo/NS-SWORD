...
Morsink::Morsink(const double& req_nounits, const double& mass_nounits, const double& omega_nounits)
...
Morsink::Morsink(const double& req_nounits, const double& mass_nounits, const double& omega_nounits, int model )
  : OblModelBase(req_nounits, mass_nounits, omega_nounits), zeta(zetaparam(mass_nounits, req_nounits)), eps(epsparam(omega_nounits, mass_nounits, req_nounits)), model(model) {
    ...
    while ((std::getline(file, line)) && (!found)) {
        ...
        if (csv_model == model) {
            found = true;
            trim(tokens[1]);
            model_name = "Morsink_" + tokens[1];  // model name
...
Morsink::Morsink(const double& req_nounits, const double& mass_nounits, const double& omega_nounits, std::string model_name)
  : OblModelBase(req_nounits, mass_nounits, omega_nounits), zeta(zetaparam(mass_nounits, req_nounits)), eps(epsparam(omega_nounits, mass_nounits, req_nounits)) {
    ...
    while ((std::getline(file, line)) && (!found)) {
        ...
        if (csv_model_name == model_name) {
            found = true;
            this->model_name = "Morsink_" + csv_model_name;  // Assign trimmed model name
...
bool Morsink::validModel(int model) {
    bool found = false;
    // Load in the csv
    std::ifstream file(coefficients_csv);
    std::string line;
...
double Morsink::R_at_costheta( const double& costheta ) const {
...
double Morsink::Dtheta_R( const double& costheta )  {
  // Return dR(theta) / dtheta in "nounits".
  // note that the user supplies cos(theta) and not theta.
  if(costheta == 0.0 || fabs(costheta) == 1.0) return double(0.0);
  else {
    // -sin(theta) * Dcostheta(R)
    double calculation = double( -sqrt(1.0-costheta*costheta) // -sin(theta). Our theta runs from 0 to pi; so sin(theta) is always positive, so this is acceptable.
		   * Req_nounits()
		   *( a0()*Dmu_P0(costheta) + a2()*Dmu_P2(costheta) + a4()*Dmu_P4(costheta) )
		   );
    //std::cout << costheta << "," << calculation << std::endl;
    return calculation;
  }
    
}

double Morsink::zetaparam( const double& Mass_nounits, const double& Req_nounits ) {
  return double( Mass_nounits / Req_nounits );
}

double Morsink::epsparam( const double& Omega_nounits, const double& Mass_nounits, const double& Req_nounits ) {
  return double( pow(Omega_nounits,2.0) * pow(Req_nounits,3.0) / Mass_nounits );
}

double Morsink::get_zeta() const {
  return double(zeta);
}

double Morsink::get_eps() const {
  return double(eps);
}
...
extern "C" OblModelBase* createOblModelBase(const double& req_nounits, const double& mass_nounits, const double& omega_nounits, int model) {
    return new Morsink(req_nounits, mass_nounits, omega_nounits, model);
}
...
extern "C" OblModelBase* createOblModelBaseStr(const double& req_nounits, const double& mass_nounits, const double& omega_nounits, std::string model_name) {
    return new Morsink(req_nounits, mass_nounits, omega_nounits, model_name);
}
