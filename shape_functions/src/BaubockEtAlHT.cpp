...
BaubockOblModel::BaubockOblModel(double req_nounits, double mass_nounits, double omega_nounits):OblModelBase(req_nounits, mass_nounits, omega_nounits) {
  Req_HT = findReqHT(req_nounits);
  model_name = "Baubock_HT";
}

// Constants
const double G = Units::G * 1e-3;  // Gravitational constant in m^3/kg/s^2
const double c = Units::C / 100;        // Speed of light in m/s
const double ErrorEpsilon = 1e-8; //How much I allow my req_HT to differ from our intended req.

double BaubockOblModel::findReqHT(double req_nounits) {
  Req_HT = req_nounits;
  double factor = 0.2;

  //Todo: This isn't working the way I intended. The result is now much too small! Keep trying.

  while (req_nounits - R_at_costheta(0) > ErrorEpsilon) { //Returned value is smaller than what it should be.
    //Exponential search upwards for the Req_HT value that gets within the epsilon without getting smaller than req_nounits.
    Req_HT *= (1+factor);
    if (req_nounits - R_at_costheta(0) < 0) {
      Req_HT /= (1+factor);
      factor /= 2.0;
    }
  }
  while (R_at_costheta(0) - req_nounits > ErrorEpsilon) { //Returned value is bigger than what it should be.
    //Exponential search downwards for the Req_HT value that gets within the epsilon without getting smaller than req_nounits.
    Req_HT *= (1-factor);
    if (R_at_costheta(0) - req_nounits < 0) {
      Req_HT /= (1-factor);
      factor /= 2.0;
    }
  }
...
double dR_dtheta_HT(double M_0, double R_0, double f, double theta) {
  double zeta_val = zeta(M_0, R_0);
  //std::cout << "zeta(M_0, R_0): " << zeta_val << std::endl;
  double f_0_val = f_0(M_0, R_0);
  //std::cout << "f_0(M_0, R_0): " << f_0_val << std::endl;
...
double d_internal_function(double M_0, double R_0, double f, double theta) {
  double dradius_HT = dR_dtheta_HT(M_0, R_0, f, theta);
  double radius_HT = R_theta_HT(M_0, R_0, f, theta);
  double GM_c2 = G * M_0 / pow(c, 2);
...
double BaubockOblModel::R_at_costheta( const double& costheta ) const {
  ...
  double radius_theta_ht = R_theta_HT(mass_mks, radius_mks, f_mks, theta);

  //Step 4: Turn the value to cgs, and then to unitless.
  double radius_theta_ht_cgs = radius_theta_ht * 100;
  double radius_theta_ht_unitless = Units::cgs_to_nounits(radius_theta_ht_cgs, Units::LENGTH);

  //Step 5: Return this value.
  return radius_theta_ht_unitless;
}
...
double BaubockOblModel::Dtheta_R( const double& costheta )  {
  ...
  //Step 3: Use R_theta_BL(double M_0, double R_0, double f, double theta) to get the radii.
  double drdt = dR_dtheta_HT(mass_mks, radius_mks, f_mks, theta);
